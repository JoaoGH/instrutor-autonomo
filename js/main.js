...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const CONTENT = {
    contact: {
      ...
      defaultMessage:
        'Olá, Instrutor Hélvio! Gostaria de informações sobre as aulas práticas em Sapiranga e região.',
    },
    toastActivities: [
  ...
  let selectedService =
    'Treinamento para Habilitados (Perder o Medo de Dirigir em Sapiranga/região)';
  ...
  function updateWhatsAppPreview() {
    ...
    const composedMessage =
      `Olá, Instrutor Hélvio! Meu nome é *${name}*.\n\n` +
  ...
  serviceOptions.forEach((card) => {
    card.addEventListener('click', () => {
      serviceOptions.forEach((c) =>
        c.classList.remove('selected', 'border-brand-600', 'bg-brand-50')
      );
  ...
  if (studentTransmissionSelect)
    studentTransmissionSelect.addEventListener('change', updateWhatsAppPreview);
